...
abstract class BaseTransactionManager {
  ...
  final PayInitializer initializer = Repository.instance.initializer;
  final transactionBloc = TransactionBloc.instance;
  final connectionBloc = ConnectionBloc.instance;
  Payload payload;
  String flwRef;
  bool saveCard = true;
  Function(bool, BankCard, PayInitializer) onPaymentSuccess;
  ...
  reQueryTransaction({ValueChanged<ReQueryResponse> onComplete}) async {
    ...
    try {
      var response =
          await service.reQuery(payload.txRef, PaymentKeys.secretKey);
      onComplete(response);
